...
class PixelNorm(nn.Module):
    ...
    def forward(self, input):
        return input * torch.rsqrt(torch.mean(input ** 2, dim=self.dim, keepdim=True) + 1e-8)
...
class Generator(nn.Module):
    ...
    def forward(
        ...
        if not input_is_stylespace:
            ...
            if attention_map is None:
                ...
                if layer == attention_layer:
                    this_layer = True
                    scale_attention_map = F.interpolate(attention_map, out.shape[-1]).repeat(1, out.shape[1], 1, 1)
                    out = scale_attention_map * out + (1 - scale_attention_map) * feature_map[layer - 1]
                new_feature_map.append(out)
            style_vector.append(out_style)

            skip, out_style = self.to_rgb1(out, latent[:, 1])
            if attention_map is None:
                feature_map.append(skip)
            else:
                layer += 1
                if layer == attention_layer or this_layer:
                    this_layer = False
                    scale_attention_map = F.interpolate(attention_map, skip.shape[-1]).repeat(1, skip.shape[1], 1, 1)
                    skip = scale_attention_map * skip + (1 - scale_attention_map) * feature_map[layer - 1]
                new_feature_map.append(skip)
            style_vector.append(out_style)
        ...
        if return_latents:
            ...
            if attention_map is None:
                return image, latent, style_vector, feature_map
            else:
                return image, latent, style_vector, new_feature_map
        else:
            return image, None
